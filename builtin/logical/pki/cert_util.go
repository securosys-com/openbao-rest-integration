...
func (sc *storageContext) fetchCAInfoByIssuerId(issuerId issuerID, usage issuerUsage) (*certutil.CAInfoBundle, error) {
	...
	if parsedBundle.PrivateKey == nil && parsedBundle.PrivateKeyType != "" {
...
func signCert(b *backend,
	...
	caSign *certutil.CAInfoBundle,
	isCA bool,
	useCSRValues bool, keyEntry *keyEntry) (*certutil.ParsedCertBundle, []string, error,
) {
	if data.role == nil {
		return nil, nil, errutil.InternalError{Err: "no role found in data bundle"}
	}
	...
	if keyEntry != nil && keyEntry.ExternalName != "" {
		parsedBundle, err = SignCertificateWithoutPrivateKey(creation, rand.Reader, keyEntry)
	} else {
