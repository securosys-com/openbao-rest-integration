...
WRITE:
	...
	var crlBytes []byte
	if signingBundle.ParsedCertBundle.PrivateKeyType == "" {
		issuer, err := sc.fetchIssuerById(thisIssuerId)
		keyEntry, err := sc.fetchKeyById(issuer.KeyID)
		if err == nil {
			crlBytes, err = CreateRevocationList(rand.Reader, revocationListTemplate, signingBundle.Certificate, keyEntry)
		}
	} else {
