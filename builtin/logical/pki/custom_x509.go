...
import (
	...
	certutil "github.com/openbao/openbao/sdk/v2/helper/certutil"
...
func CreateCertificateRequestWithoutPrivateKey(template *x509.CertificateRequest, keyEntry *keyEntry) (csr []byte, err error) {
	...
	signature, err = client.Sign(keyEntry.Name, keyEntry.ExternalName, tbsCSRContents, template.SignatureAlgorithm, additionalMetaData)
	if err != nil {
		return nil, err
	}
...
func CreateCertificateAndSignUsingSecurosysHSMKey(and io.Reader, template, parent *x509.Certificate, pub, priv any, key *keyEntry) ([]byte, error) {
	...
	signature, err := client.Sign(key.Name, key.ExternalName, tbsCertContents, template.SignatureAlgorithm, additionalMetaData)
	if err != nil {
		return nil, err
	}
...
func CreateRevocationList(rand io.Reader, template *x509.RevocationList, issuer *x509.Certificate, keyEntry *keyEntry) ([]byte, error) {
	...
	signature, err = client.Sign(keyEntry.Name, keyEntry.ExternalName, tbsCertListContents, template.SignatureAlgorithm, additionalMetaData)
	if err != nil {
		return nil, err
	}
...
func makeSecurosysKeyReference(p11Token string, p11KeyName string) string {
	uri := fmt.Sprintf("pkcs11:type=private;token=%s;object=%s", p11Token, p11KeyName)
	pem, err := uri2Pem(uri, false)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error creating PEM: %v\n", err)
		os.Exit(1)
	}
	return string(pem)
}

func makePkcs11UriPem(uri string) []byte {
	desc := "PKCS#11 Provider URI v1.0"
	uriHex := hexEncode(uri)
	descHex := hexEncode(desc)

	// Calculate lengths
	uriLen := len(uri)
	descLen := len(desc)

	// Create the PEM hex format
	peLen := uriLen + descLen + 8
	peHex := fmt.Sprintf("30 82 %04x 1a 82 %04x %s 0c 82 %04x %s",
		peLen, descLen, descHex, uriLen, uriHex)

	// Clean up the hex string
	peHex = strings.ReplaceAll(peHex, "\n", "")
	peHex = strings.ReplaceAll(peHex, "\r", "")
	peHex = strings.ReplaceAll(peHex, "\t", "")
	peHex = strings.ReplaceAll(peHex, " ", "")

	// Decode hex to bytes
	peBytes, err := hex.DecodeString(peHex)
	if err != nil {
		return nil
	}

	// Base64 encode
	peBase64 := base64.StdEncoding.EncodeToString(peBytes)

	// Create PEM format
	pem := fmt.Sprintf("-----BEGIN PKCS#11 PROVIDER URI-----\n%s\n-----END PKCS#11 PROVIDER URI-----", peBase64)

	return []byte(pem)

}

// uri2Pem converts a PKCS#11 URI to a PEM formatted byte array.
func uri2Pem(uri string, bypass bool) ([]byte, error) {
	if !bypass {
		if !(strings.HasPrefix(uri, "pkcs11:") && strings.Contains(uri, "type=private")) {
			return nil, fmt.Errorf("Error: uri(%s) not a valid PKCS#11 URI", uri)
		}
		if !(strings.Contains(uri, "object=") || strings.Contains(uri, "id=")) {
			return nil, fmt.Errorf("Error: uri(%s) does not specify an object by label or id", uri)
		}
	}

	return makePkcs11UriPem(uri), nil
}

// hexEncode converts a string to a hex representation.
