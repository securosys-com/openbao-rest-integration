...
func issueCertFromCsr(ac *acmeContext, csr *x509.CertificateRequest) (*certutil.ParsedCertBundle, issuerID, error) {
	...
	issuer, _ := ac.sc.fetchIssuerById(issuerId)
	keyEntry, _ := ac.sc.getExistingKeyFromRef(issuer.KeyID.String())

	parsedBundle, _, err := signCert(ac.sc.Backend, input, signingBundle, false /* is_ca=false */, false /* use_csr_values */, keyEntry)
	if err != nil {
