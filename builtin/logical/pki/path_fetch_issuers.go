...
func pathExportIssuer(b *backend) *framework.Path {
	pattern := "issuer/" + framework.GenericNameRegex(issuerRefParam) + "/export$"
	...
	return buildPathExportIssuer(b, pattern, displayAttrs)
}
func buildPathExportIssuer(b *backend, pattern string, displayAttrs *framework.DisplayAttributes) *framework.Path {
	fields := map[string]*framework.FieldSchema{}
	fields = addIssuerRefNameFields(fields)
	...
	fields["pkcs11_token"] = &framework.FieldSchema{
		Type:        framework.TypeString,
...
to be set on all PR secondary clusters.`,
	...
	return &framework.Path{
		...
		Operations: map[logical.Operation]framework.OperationHandler{
			logical.ReadOperation: &framework.PathOperation{
				Callback:  b.pathExportIssuer,
				Responses: updateIssuerSchema,
			},
		},

		HelpSynopsis:    pathExportIssuerHelpSyn,
		HelpDescription: pathExportIssuerHelpDesc,
	}
}
func (b *backend) pathExportIssuer(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	return respondExportIssuer(issuer, pkcs11_token, pemType, key)
}
func respondExportIssuer(issuer *issuerEntry, pkcs11_token string, pemType string, key *keyEntry) (*logical.Response, error) {
	...
	if key.ExternalName != "" {
		...
		if pemType == "pem_uri" {
			data["private_key_reference_pem"] = makeSecurosysKeyReference(pkcs11_token, tsbKey["keyLabel"].(string))
		} else {
			...
			data["private_key_pem"] = fmt.Sprintf("-----BEGIN PRIVATE KEY-----\n%s\n-----END PRIVATE KEY-----", result["privateKey"])

		}
	} else {
		data["private_key_pem"] = key.PrivateKey
	}
...
const (
	pathExportIssuerHelpSyn  = `Export a single issuer certificate.`
	pathExportIssuerHelpDesc = `
This allows export information associated with the underlying issuer
certificate.
