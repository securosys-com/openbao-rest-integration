...
func pathImportIssuer(b *backend) *framework.Path {
	return &framework.Path{
		Pattern: "issuers/import/(cert|bundle|securosys-hsm)",

		DisplayAttrs: &framework.DisplayAttributes{
			OperationPrefix: operationPrefixPKIIssuers,
			OperationVerb:   "import",
			OperationSuffix: "cert|bundle|securosys-hsm",
		},
...
func (b *backend) pathImportIssuers(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	keysAllowed := strings.HasSuffix(req.Path, "bundle") || strings.HasSuffix(req.Path, "securosys-hsm") || req.Path == "config/ca"
	...
	keyName, _ := getKeyName(sc, data)
	managedKeyName, _ := getManagedKeyId(data)
	var cert *x509.Certificate
	if len(issuers) == 1 {
		block, _ := pem.Decode([]byte(issuers[0]))
	...
	issuerName = strings.ReplaceAll(issuerName, " ", "_")
	issuerName = strings.ToLower(issuerName)
	importSecurosysHSM := false
	if keyName == "" {
		keyName = "imported_" + issuerName + "_privatekey"
	}
	...
	if strings.HasSuffix(req.Path, "/securosys-hsm") {
		...
		issuerName = "imported_" + issuerName
	} else {
		keyName = ""
		externalKeyName = ""
		issuerName = ""
	...
	for keyIndex, keyPem := range keys {
		...
		key, existing, err := importKeyFromBytes(sc, keyPem, keyName, externalKeyName, importSecurosysHSM, extractable)
		if err != nil {
