...
for that. Defaults to true.`,
			...
			"external_key": {
				Type:        framework.TypeBool,
				Default:     false,
				Description: `ExternalKey generation using securosys-hsm plugin`,
			},
			"external_key_name": {
				Type:        framework.TypeString,
				Default:     "",
				Description: `Securosys-hsm key name`,
			},
...
func (b *backend) pathRoleCreate(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	entry := &roleEntry{
		...
		ExternalKey:                   data.Get("external_key").(bool),
		ExternalKeyName:               data.Get("external_key_name").(string),
...
type roleEntry struct {
	...
	ExternalKey                   bool          `json:"external_key"`
	ExternalKeyName               string        `json:"external_key_name"`
