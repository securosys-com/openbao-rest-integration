...
func (b *backend) pathIssuerSignIntermediate(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	*role.AllowWildcardCertificates = true
	...
	if issuer != nil {
		if issuer.KeyID.String() == "" {
			block, _ := pem.Decode([]byte(issuer.CAChain[0]))
			cert, _ := x509.ParseCertificate(block.Bytes)
			issuerId, _ := sc.resolveIssuerReference(cert.Subject.CommonName)
			issuer, _ := sc.fetchIssuerById(issuerId)
	...
	parsedBundle, warnings, err := signCert(b, input, signingBundle, true, useCSRValues, keyEntry)
	if err != nil {
		switch err.(type) {
		case errutil.UserError:
			return logical.ErrorResponse(err.Error()), nil
