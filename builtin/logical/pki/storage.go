...
func (e keyEntry) getPublicKey() crypto.PublicKey {
	return ParsePublicKeyString(e.PublicKey)
}
...
func ParsePublicKeyString(publicKey string) crypto.PublicKey {
	...
	return pkForImportingKey
}

func (sc *storageContext) importKeyWithoutPrivateKey(externalName string, publicKeyValue string, keyName string, privateKeyValue string) (*keyEntry, bool, error) {
	...
	pkForImportingKey = ParsePublicKeyString(publicKeyValue)
	...
	for _, identifier := range knownKeys {
		...
		if keyName != "" && existingKey.Name == keyName && existingKey.PrivateKey == "" {
			return existingKey, true, nil
		}
	...
	result.ID = genKeyId()
	result.Name = keyName
	result.PrivateKey = privateKeyValue
	result.PrivateKeyType = certutil.UnknownPrivateKey
	result.ExternalName = externalName
	result.PublicKey = string(WrapPublicKeyWithHeaders(publicKeyValue)[:])
...
func (sc *storageContext) importKey(keyValue string, keyName string, keyType certutil.PrivateKeyType) (*keyEntry, bool, error) {
	...
	for _, identifier := range knownKeys {
		...
		if keyName != "" && existingKey.Name == keyName && existingKey.PrivateKey == "securosys_hsm" && pkForImportingKey == nil {
			return existingKey, true, nil
		}
...
func (sc *storageContext) writeCaBundle(caBundle *certutil.CertBundle, issuerName string, keyName string) (*issuerEntry, *keyEntry, error) {
	...
	if err != nil && caBundle.PrivateKey != "" {
		return nil, nil, err
	}
