...
func configureWrapper(configKMS *KMS, infoKeys *[]string, info *map[string]string, logger hclog.Logger, opts ...wrapping.Option) (wrapping.Wrapper, error) {
	...
	case "securosys-hsm":
		wrapper, kmsInfo, err = GetSecurosysKMSFunc(configKMS, opts...)
...
var GetSecurosysKMSFunc = func(kms *KMS, opts ...wrapping.Option) (wrapping.Wrapper, map[string]string, error) {
	wrapper := hsm.NewWrapper()
	wrapperInfo, err := wrapper.SetConfig(context.Background(), append(opts, wrapping.WithConfigMap(kms.Config))...)
	if err != nil {
		// If the error is any other than logical.KeyNotFoundError, return the error
		if !errwrap.ContainsType(err, new(logical.KeyNotFoundError)) {
	...
	info := make(map[string]string)
	if wrapperInfo != nil {
		info["TSB Api Endpoint"] = wrapperInfo.Metadata["tsb_api_endpoint"]
		info["Check Every"] = wrapperInfo.Metadata["check_every"]
		info["Authorization"] = wrapperInfo.Metadata["auth"]
		info["Approval Timeout"] = wrapperInfo.Metadata["approval_timeout"]
		info["Securosys Key Label"] = wrapperInfo.Metadata["key_label"]
	}
	return wrapper, info, nil
}
